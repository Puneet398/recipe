...
class RecipeScraper:
    ...
    def parse_with_ai(self, scraped_data):
        ...
        # Special handling for OCR/photo content
        ...
        try:
            response = self.ai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a recipe extraction expert specializing in converting cooking content into clean, minimalist, recipes. Your priority is capturing ALL cooking steps and ingredients without omission. Focus on thoroughness, concise summaries and accuracy."
                    },
                    {
                        "role": "user",
                        "content": prompt
...
Could not extract a clear recipe from this URL. The page may not contain a recipe or may be behind a paywall."""
    ...
    def scrape_and_save(self, url):
        ...
        # Extract recipe name from markdown
...
# HTML template with OCR functionality and edit/delete
...
def get_recipes():
    """Get list of all markdown recipe files"""
    try:
        recipe_files = glob.glob('recipe_*.md')
        recipes = []
        ...
        recipes.sort(key=lambda x: x['created'], reverse=True)
...
def save_recipe():
    """Save updated recipe content to file"""
    try:
        ...
        # Security check - only allow recipe files
...
def get_recipe_content(filename):
    """Get content of a specific recipe file"""
    try:
        # Security check - only allow recipe files
        if not filename.startswith('recipe_') or not filename.endswith('.md'):
            return jsonify({'error': 'Invalid filename'}), 400
...
def process_ocr_text():
    """Process OCR-extracted text and convert to recipe format"""
    try:
        data = request.get_json()
        ...
        # Create a mock scraped_data structure for OCR text
        scraped_data = {
            'url': 'Photo Upload',
        ...
        # Generate filename for photo-based recipe
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
...
if __name__ == '__main__':
    ...
    print("📷 Now supports OCR from photos!")
    print("✏️ Edit and delete recipes with ease!")
